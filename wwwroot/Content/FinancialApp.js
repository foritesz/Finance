...
{
 "use strict";
 var FinancialApp,Client,WebSharper,Operators,Obj,UI,Var$1,Var,HtmlModule,attr,List,Transaction,System,Guid,TransactionType,Category,Client$1,Templates,Doc,JavaScript,Pervasives,ConcreteVar,Snap,Seq,Stock,Collections,Map,Portfolio,AttrProxy,T,Arrays,AttrModule,FSharpMap,Decimal,EventTarget,Node,JS,Dictionary,Unchecked,Abbrev,Fresh,SC$1,Numeric,Enumerator,Random,BalancedTree,Pair,Attrs,DomUtility,View,SC$2,SC$3,HashSet,WindowOrWorkerGlobalScope,Docs,T$1,Tree,Array,BindVar,DictionaryUtil,Element,Object,Prepare,Slice,KeyCollection,DocElemNode,CharacterData,An,Settings,Mailbox,SC$4,Elt,SC$5,Error,ArgumentException,Strings,Attrs$1,Dyn,Docs$1,RunState,NodeSet,Concurrency,Anims,SC$6,MapUtil,Updates,DynamicAttrNode,String,CheckedInput,SC$7,SC$8,AppendList,HashSetUtil,Char,DateUtil,KeyNotFoundException,Scheduler,Easing,AsyncBody,SC$9,CT,HashSet$1,Queue,CancellationTokenSource,DomNodes,OperationCanceledException,Lazy,SC$10,LazyExtensionsProxy,LazyRecord,Date,console,math,Math,Runtime;
 FinancialApp=Global.FinancialApp=Global.FinancialApp||{};
 Client=FinancialApp.Client=FinancialApp.Client||{};
 ...
 TransactionType=Client.TransactionType=Client.TransactionType||{};
 Category=Client.Category=Client.Category||{};
 ...
 Stock=Client.Stock=Client.Stock||{};
 Collections=WebSharper.Collections=WebSharper.Collections||{};
 Map=Collections.Map=Collections.Map||{};
 Portfolio=Client.Portfolio=Client.Portfolio||{};
 ...
 SC$1=Global.StartupCode$FinancialApp$Client=Global.StartupCode$FinancialApp$Client||{};
 ...
 SC$2=Global.StartupCode$WebSharper_MathJS_Extensions$Decimal=Global.StartupCode$WebSharper_MathJS_Extensions$Decimal||{};
 ...
 Client.app=function()
 {
  var dashboard,portfolioView,transactionType,transactionCategory,transactionAmount,stockSymbol,stockQuantity;
  function updateViews()
  {
   dashboard.Set(Client.displayDashboard());
   portfolioView.Set(Client.displayPortfolio());
  }
  function addTransactionFromInput()
  {
   var m,s;
   Client.addTransaction(Transaction.New(Guid.NewGuid(),transactionType.Get()=="Income"?TransactionType.Income:TransactionType.Expense,(m=transactionCategory.Get(),m=="Food"?Category.Food:m=="Utilities"?Category.Utilities:m=="Salary"?Category.Salary:m=="Entertainment"?Category.Entertainment:{
    $:4,
    $0:transactionCategory.Get()
   }),(s=transactionAmount.Get(),Decimal.WSDecimalMath().bignumber(s)),Date.now()));
   updateViews();
  }
  function addStockFromInput()
  {
   Client.addStock(stockSymbol.Get(),Operators.toInt(Global.Number(stockQuantity.Get())));
   updateViews();
  }
  function removeStockFromInput()
  {
   Client.removeStock(stockSymbol.Get(),Operators.toInt(Global.Number(stockQuantity.Get())));
   updateViews();
  }
  dashboard=Var$1.Create$1(Client.displayDashboard());
  portfolioView=Var$1.Create$1(Client.displayPortfolio());
  transactionType=Var$1.Create$1("Income");
  transactionCategory=Var$1.Create$1("Salary");
  transactionAmount=Var$1.Create$1("");
  stockSymbol=Var$1.Create$1("");
  stockQuantity=Var$1.Create$1("");
  return Doc.Element("div",[AttrProxy.Create("class","container")],[Doc.Element("h1",[],[Doc.TextNode("Finance Manager")]),Doc.Element("div",[AttrProxy.Create("class","budget-form")],[Doc.Element("div",[AttrProxy.Create("class","add-budget")],[Doc.Element("h3",[],[Doc.TextNode("Add Transaction")]),Doc.Element("label",[],[Doc.TextNode("Type: ")]),Doc.Select([],Global.id,List.ofArray(["Income","Expense"]),transactionType),Doc.Element("label",[],[Doc.TextNode("Category: ")]),Doc.Select([],Global.id,List.ofArray(["Food","Utilities","Salary","Entertainment","Others"]),transactionCategory),Doc.Element("br",[],[]),Doc.Element("label",[],[Doc.TextNode("Amount: ")]),Doc.Input([],transactionAmount),Doc.Element("button",[AttrModule.Handler("click",function()
  {
   return function()
   {
    return addTransactionFromInput();
   };
  })],[Doc.TextNode("Add Transaction")])]),Doc.Element("div",[AttrProxy.Create("class","add-expense")],[Doc.Element("h3",[],[Doc.TextNode("Manage Stocks")]),Doc.Element("label",[],[Doc.TextNode("Subject: ")]),Doc.Input([],stockSymbol),Doc.Element("label",[],[Doc.TextNode("Quantity: ")]),Doc.Input([],stockQuantity),Doc.Element("button",[AttrModule.Handler("click",function()
  {
   return function()
   {
    return addStockFromInput();
   };
  })],[Doc.TextNode("Add Stock")]),Doc.Element("button",[AttrProxy.Create("class","reset-all"),AttrModule.Handler("click",function()
  {
   return function()
   {
    return removeStockFromInput();
   };
  })],[Doc.TextNode("Remove Stock")])])]),Doc.Element("div",[],[Doc.BindView(Global.id,dashboard.get_View())]),Doc.Element("div",[],[Doc.BindView(Global.id,portfolioView.get_View())])]);
 };
 Client.displayDashboard=function()
 {
  var res,expenses,res$1,netIncome,l,e,a,_this,income,l$1,e$1,a$1,_this$1;
  try
  {
   l=List.filter(function(t)
   {
    return t.Type.$===0;
   },Client.transactions().Get());
   res=Decimal.CreateDecimal(0,0,0,false,0);
   e=Enumerator.Get(l);
   try
   {
    while(e.MoveNext())
     {
      res=(a=e.Current().Amount,_this=Decimal.WSDecimalMath(),_this.add.apply(_this,[res,a]));
     }
   }
   finally
   {
   ...
   income=res;
   l$1=List.filter(function(t)
   {
    return t.Type.$===1;
   },Client.transactions().Get());
   res$1=Decimal.CreateDecimal(0,0,0,false,0);
   e$1=Enumerator.Get(l$1);
   try
   {
    while(e$1.MoveNext())
     {
      res$1=(a$1=e$1.Current().Amount,_this$1=Decimal.WSDecimalMath(),_this$1.add.apply(_this$1,[res$1,a$1]));
     }
   }
   finally
   {
   ...
   expenses=res$1;
   netIncome=Decimal.WSDecimalMath().subtract(income,expenses);
   return Doc.Element("div",[AttrProxy.Create("class","budget-details")],[Doc.Element("h3",[],[Doc.TextNode("Current Financial Status")]),Doc.Element("div",[AttrProxy.Create("class","totals")],[Doc.Element("p",[],[Doc.TextNode("Total Income: "+Global.String(income)+"$")]),Doc.Element("p",[],[Doc.TextNode("Total Expenses: "+Global.String(expenses)+"$")]),Doc.Element("p",[],[Doc.TextNode("Net Income: "+Global.String(netIncome)+"$")])])]);
  }
  catch(ex)
  {
   console.log("Error in displayDashboard: ",ex.message);
   return Doc.Element("div",[],[Doc.TextNode("Error displaying dashboard")]);
  }
 };
 Client.displayPortfolio=function()
 {
  return Doc.Element("div",[AttrProxy.Create("class","budget-details")],[Doc.Element("h3",[],[Doc.TextNode("Current Portfolio")]),Doc.Element("ul",[AttrProxy.Create("class","list-group")],List.ofSeq(Seq.delay(function()
  {
   return Seq.collect(function(m)
   {
    var stock,price;
    stock=m[1];
    price=Client.getRandomStockPrice(m[0]);
    return[Doc.Element("li",[AttrProxy.Create("class","list-group-item")],[Doc.TextNode("Symbol: "+Global.String(stock.Symbol)+", Quantity: "+Global.String(stock.Quantity)+", Current Price: "+Global.String(price)+"$")])];
   },Map.ToSeq(Client.portfolio().Get().Stocks));
  })))]);
 };
 Client.removeStock=function(symbol,quantity)
 {
  var m,s;
  m=Client.portfolio().Get().Stocks.TryFind(symbol);
  if(m!=null&&m.$==1)
   m.$0.Quantity>quantity?(s=m.$0,Client.portfolio().Set((Client.portfolio().Get(),Portfolio.New(Client.portfolio().Get().Stocks.Add$1(symbol,Stock.New(s.Symbol,s.Quantity-quantity)))))):m.$0.Quantity===quantity?Client.portfolio().Set((Client.portfolio().Get(),Portfolio.New(Client.portfolio().Get().Stocks.Remove$1(symbol)))):void 0;
 };
 Client.addStock=function(symbol,quantity)
 {
  var stock,m,s;
  stock=(m=Client.portfolio().Get().Stocks.TryFind(symbol),m==null?Stock.New(symbol,quantity):(s=m.$0,Stock.New(s.Symbol,s.Quantity+quantity)));
  Client.portfolio().Set((Client.portfolio().Get(),Portfolio.New(Client.portfolio().Get().Stocks.Add$1(symbol,stock))));
 };
 Client.addTransaction=function(transaction)
 {
  Client.transactions().Set(new T({
   $:1,
   $0:transaction,
   $1:Client.transactions().Get()
  }));
 };
 Client.transactions=function()
 {
  SC$1.$cctor();
  return SC$1.transactions;
 };
 Client.getRandomStockPrice=function(symbol)
 {
  var v;
  v=Client.random().Next(100,1000);
  return Decimal.WSDecimalMath().bignumber(v);
 };
 Client.portfolio=function()
 {
  SC$1.$cctor();
  return SC$1.portfolio;
 };
 ...
 Client.createMathInstance=function()
 {
  return math.create({
   number:"BigNumber",
   precision:29,
 ...
 Transaction.New=function(Id,Type,Category$1,Amount,Date$1)
 {
  return{
   Id:Id,
   Type:Type,
 ...
 Stock.New=function(Symbol,Quantity)
 {
  return{
   Symbol:Symbol,
   Quantity:Quantity
 ...
 SC$1.$cctor=function()
 {
  ...
  SC$1.mathInstance=Client.createMathInstance();
  SC$1.random=new Random.New();
  SC$1.transactions=Var$1.Create$1(T.Empty);
  SC$1.portfolio=Var$1.Create$1(Portfolio.New(new FSharpMap.New([])));
 };
 ...
 SC$2.$cctor=function()
 {
  ...
  SC$2.WSDecimalMath=math.create((r={},r.number="BigNumber",r.precision=29,r.predictable=true,r.epsilon=1E-60,r));
 };
 SC$3.$cctor=function()
 {
